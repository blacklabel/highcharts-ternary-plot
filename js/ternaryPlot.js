...
//
//function TernaryPlotPlugin(Highcharts: typeof Highcharts): void {
function TernaryPlotPlugin(H) {
    const seriesType = H.seriesType, wrap = H.wrap, merge = H.merge, Series_correctFloat = H.correctFloat, pick = H.pick, Series_fireEvent = H.fireEvent;
    /***
    ...
    ***/
    wrap(H.Chart.prototype, 'init', function (p, userOptions, callback) {
        if (userOptions.chart.ternary) {
            userOptions.chart.ternarySpacing = pick(userOptions.chart.ternarySpacing, 25);
        }
    ...
    ***/
    H.wrap(H.Chart.prototype, 'firstRender', function (p) {
        const chart = this, options = chart.options, defaultTernary = {
            tickInterval: 50,
        ...
        if (options.chart.ternary && chart.ternaryAxis) {
            chart.ternaryAxis.forEach(function (axis, i) {
                if (axis.title && axis.title.text) {
                    if (!axis.titleElem) {
                        axis.titleElem = chart.renderer.text(axis.title.text, 0, 0).css(axis.title.style).attr(axis.title.style).add();
                    }
                ...
                if (axis.gridlineWidth >= 1) {
                    axis.gridlineTicks = chart.getGrids(i, axis.gridlineWidth, axis.tickInterval);
                }
                if (axis.minorGridlineWidth >= 1) {
                    axis.minorGridlineTicks = chart.getGrids(i, axis.minorGridlineWidth, axis.minorTickInterval);
                }
                axis.gridlineLabels = chart.getLabels(axis, i, axis.gridLineWidth, axis.tickInterval);
            });
        }
    });
    /***
    ...
    ***/
    H.Chart.prototype.getGrids = function (index, width, interval) {
        const chart = this, plotTop = chart.plotTop, ticks = {};
        let cursor = 0, pos, posEnd, tick;
        while (cursor <= 100) {
            if (index === 1) {
    ...
    ***/
    H.Chart.prototype.getLabels = function (axis, index, width, interval) {
        const chart = this, plotTop = chart.plotTop, labels = {}, distance = 20;
        let cursor = 0, pos, offsetY, offsetX;
        while (cursor <= 100) {
            if (index === 2) {
                // vertical left
            ...
            labels[cursor] = chart.renderer.text(cursor, pos[0] + offsetX, pos[1] + plotTop + offsetY).attr({
    ...
    ***/
    H.Chart.prototype.toPerspective = function (point) {
        const chart = this, scale = pick(chart.options.ternaryRange, 100), spacing = chart.ternarySpacing * 2;
        ...
        const a = pick(point.x, point[0]) * width / scale;
        const b = pick(point.y, point[1]) * width / scale;
        return [
    ...
    seriesType('ternaryscatter', 'scatter', {
        ...
        translate() {
            ...
            xAxis = series.xAxis, points = series.points, dataLength = points.length, pointPlacement = series.pointPlacementToXValue(), // #7860
